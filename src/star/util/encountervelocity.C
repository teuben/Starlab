...
main(int argc, char **argv)
{
    ...
    extern char *poptarg;
    int c;
    const char *param_string = "n:Mmeor:v";	// Note: "v" removed because only the
    ...
    real *pvenc = new real[nvzones];
    for (int i=0; i<nvzones; i++)
      pvenc[i] = 0;
	...
	for (int jm=im; jm<nzones+1; jm++) {
	  ...
	  // Compute the probability distribution for the encounter velocity
	    real dv = 40/dynamic_cast(real, nvzones);
	    for(int iv=0; iv<nvzones; iv++) // weighted on the encounter rate
	      pvenc[iv] += encounter_rate   
		         * gravitational_focussed_velocity(m_mean[jm],
							   m_mean[im], 
							   magnify_r * r_tot,
							   iv*dv, v_rel);
	  }
      ...
      real max_pvenc = 0;
      for(int iv=0; iv<nvzones; iv++)
	max_pvenc = Starlab::max(max_pvenc, pvenc[iv]);
      cerr << "Encounter velocity poribability distribution"<<endl;
      real dv = 40./dynamic_cast(real, nvzones);
      for(int iv=0; iv<nvzones; iv++) {
	pvenc[iv] = pvenc[iv]/max_pvenc;
	if (pvenc[iv]<=1e-7)
	  pvenc[iv] = 0;
	cerr << iv*dv << " " << pvenc[iv] << endl;
      }
