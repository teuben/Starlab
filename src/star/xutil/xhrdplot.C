/*
 *  xhrdplot.C:  plot an N-body system in an X environment.
 *
 *.............................................................................
 *
 ...
 *    version 5:  Aug 1996   Simon Portegies Zwart email: spz@astro.uva.nl
 *			     extension to HRD plotting.
 *
 *.............................................................................
 *
 *  non-local functions: 
...
unsigned long  hrd_win, hrd_instr, hrd_colwin;
...
float  hrd_xmin, hrd_xmax, hrd_ymin, hrd_ymax;
...
{
    // Scaling by radius is too extreme.  Use pow(log10(radius), 2)).
...
local int plot_stellar_hrd(hdyn* b, int f_flag) {
    ...
    story *st = NULL;		//star_story pointer
    stellar_type type = NAS;	//type of stellar 
    real t_cur, t_rel, m_rel, m_env, m_core, T_eff, L_eff, p_rot, b_fld;
    t_cur=t_rel=m_rel=m_env=m_core=T_eff=L_eff=p_rot=b_fld=0;// stellar parameters
    real R_eff = 0;				//extra parameter.


       // Make a list of nodes, *including* the root node if root = 1.
       // Root will be at the start of the list if it is being displayed.

        hdynptr* p = new hdynptr[n_nodes+root];

        int ip = 0;
        for_all_nodes(hdyn, b, bi) if (root || bi != b) p[ip++] = bi;
        if (ip != n_nodes+root) {
            cerr << "plot_stars: n_nodes = " << n_nodes+root
                << " counted " << ip << endl;
            exit(1);
        }
...
//                R_eff += 1;
//                point_size = (0.01 + 0.05*pow(R_eff, 2))*lmax3d/30.0;
//cerr<<"p:"<<point_size<<endl;

//	Use dynamics point size instead.
                float actual_star_size = get_point_size(R_eff);
		      actual_point_size = actual_star_size;
                ...
                if (   (r > (xmin + actual_point_size))
                    ...
                    if (bi->get_oldest_daughter()) {
                        ...
                        lux_set_color(hrd_win, lux_lookup_color(hrd_win, "grey"));
                        temp_flag =  1 - f_flag;
                    } else {
                        if (cenergy==1) {
                            char c;

                            derive_stellar_type(b, bi, c);
                            lux_set_color(win, c_energy[c]);
                            lux_set_color(hrd_win, c_energy[c]);
...
//                          ii = min((int)type, N_COLORS);

                            lux_set_color(win,c_index[ii]);
                            lux_set_color(hrd_win,c_index[ii]);
                        }
                    }

                    if (track) {
                        lux_draw_pointf(win, r, s);
                        if( (hrd_r > (hrd_xmin + actual_star_size))
                        && (hrd_r < (hrd_xmax - actual_star_size))
                        && (hrd_s > (hrd_ymin + actual_star_size))
                        && (hrd_s < (hrd_ymax - actual_star_size))  )
                           lux_draw_pointf(hrd_win, hrd_r, hrd_s);
 		    }
                    else {
                        draw_star_point(win, r, s,
                                        actual_point_size, temp_flag);

                        if( (hrd_r > (hrd_xmin + actual_star_size))
                        && (hrd_r < (hrd_xmax - actual_star_size))
                        && (hrd_s > (hrd_ymin + actual_star_size))
                        && (hrd_s < (hrd_ymax - actual_star_size)) )
                           draw_star_point(hrd_win, hrd_r, hrd_s,
                                        actual_star_size, temp_flag);
 		    }
                    if (links && bi->get_oldest_daughter()) {
                        draw_links_2d(bi, r, s);
                        draw_links_2d(bi, hrd_r, hrd_s);
 	   	    }
                }
            }
        }
...
local int plot_stars(hdyn* b, int f_flag)
{
	...
	for_all_nodes(hdyn, b, bi) if (root || bi->get_parent()) {
		...
		if (   (X > (-lmax3d + actual_point_size))
			...
			draw_star_point(win, r, s, actual_point_size, f_flag);
...
local void set_temp_buffer(int which)
{
	...
	default:		cerr << "xhrdplot: diag error...\n";
    }

    if (f < VERY_LARGE_FLOAT)
...
local void check_for_input(unsigned long win, hdyn* b,
			   bool b_flag, bool f_flag)
{
    ...
    while (lux_next_keypress(win, &key, string, &shift, &control)) {
		...
		cenergy = ((++cenergy)<=2?cenergy:0);
		update_diag_item(colorenergy);
		show_hrd_color_scheme(colwin, c_energy, c_index,
				  r_factor, cenergy, b_flag, 1);
...
",1);
	    ...
	    if (return_value == 3) update_from_dialog(b_flag);
	}
    }
}

/*-----------------------------------------------------------------------------
 *  xhrdplot.C  --  project the positions of all particles onto the screen
...
void xhrdplot(hdyn* b, float scale, int k, int d, float lmax,
	       ...
	       int init_flag)
{
	...
	if (init_status == 0) {
           ...
           initialize_hrd_graphics(r_factor, b_flag,
                                   hrd_win, hrd_instr, hrd_colwin,
                                   c_index, c_energy,
                                   win_size, hrd_xorigin, hrd_yorigin);
        }
	lux_clear_window(win);
	lux_update_fg(win);
	...
	lux_update_fg(instr);
// 		Now for the HRD
        lux_clear_window(hrd_win);
        lux_update_fg(hrd_win);
        lux_clear_window(hrd_colwin);
        lux_update_fg(hrd_colwin);
        lux_clear_window(hrd_instr);
        lux_update_fg(hrd_instr);
...
//	cenergy = ((++cenergy)<=2?cenergy:0);
	...
	set_limits(origin, lmax3d, kx, xmin, xmax, ky, ymin, ymax);
	set_hrd_limits(hrd_xmin, hrd_xmax, hrd_ymin, hrd_ymax);
	set_base_point_size(rel_point_size);
...
//		Now for the hrd/
            lux_setup_axis(hrd_win, hrd_xmin, hrd_xmax, hrd_ymin, hrd_ymax);
            draw2d_axis(hrd_win, hrd_xmin, hrd_xmax, hrd_ymin, hrd_ymax, kproj);

	// Complete the initialization with the dialog box.

	if (init_status == 0) {
           initialize_dialog(xorigin, yorigin);
           initialize_dialog(hrd_xorigin, hrd_yorigin);
        }
    ...
    // End of initialization.  Start by (re)drawing axes and instructions.

    if (track == 0) {

	lux_clear_current_region(win);
        lux_clear_current_region(hrd_win);
	if (graph3d) draw3d_axis(win, lmax3d, costheta, sintheta,
				 cosphi, sinphi);
    }

    show_main_instructions(instr, r_factor, graph3d, 1);
    show_hrd_information(hrd_instr, r_factor, graph3d, 1, b);
    ...
    int n_stars = plot_stellar_hrd(b, f_flag);
    real current_time=0;
    real system_time = b->get_time();
    current_time = b->get_starbase()->conv_t_dyn_to_star(system_time);
//    cerr<<"current_time: "<<system_time<<" "<<current_time<< endl;
    real turnoff_mass = turn_off_mass(current_time);
    ...
    if (graph3d) {
	sprintf(temp_buffer, "N = %d (Time = %5.1f) max=%5.3f",
		n_stars, system_time, lmax3d);
	lux_draw_image_string(win, 0.0, lmax3d*FAC3D, 0.5, temp_buffer, 0);
    } else {
//	sprintf(temp_buffer, "N = %d  (snapshot #%5d)", n_stars, init_flag + 1);
	sprintf(temp_buffer, "N = %d      (Time = %5.1f)", n_stars, system_time);
	lux_draw_image_string(win, (xmin+xmax)/2.0, ymax, 0.5, temp_buffer, 0);
    }
        sprintf(temp_buffer, "T = %6.1f Myr  (M_to = %5.2f Mo)", 
		current_time, turnoff_mass);
        lux_draw_image_string(hrd_win, (hrd_xmin+hrd_xmax)/2.0, 
				hrd_ymax, 0.5, temp_buffer, 0);


    // Update the display.

    update_with_delay(win, delay_time);
    update_with_delay(hrd_win, delay_time);

    // Deal with user input.

    check_for_input(win, b, b_flag, f_flag);
}

/*-----------------------------------------------------------------------------
 *  xstarplot.C  --  project the positions of all particles onto the
screen
 *                   input:   pn: a pointer to a nbody system,
 *                              k: the number of the coordinate axis
along whic
h
 *                                 the projection is directed, with
{k,x,y}
 *                                 having a right-handed orientation,
 *                           xmax: displayed x-axis spans [-xmax, xmax]
*                           ymax: displayed y-axis spans [-ymax, ymax]
...
void xstarplot(hdyn* b, float scale, int k, int d, float lmax,
               int point_mode, float rel_point_size,
               float D, int ce,
               bool b_flag, bool f_flag, bool t_flag,
               int init_flag)
{
    ...
    if (init_flag == 0) {
        ...
        if (init_status == 0) initialize_graphics(r_factor, b_flag,
                                                  win, instr, colwin,
                                                  c_index, c_energy,
                                                  win_size, xorigin, yorigin);
        lux_clear_window(win);
...
//        cenergy = ce?1:0;
	cenergy = ((++ce)<=2?ce:0);
        ...
        if (init_status == 0) initialize_dialog(xorigin, yorigin);
...
//------------------------------------------------------------------------
    ...
    int n_stars = plot_stars(b, f_flag);
...
/*-----------------------------------------------------------------------------
 *  main  --  driver to use  xhrdplot()  as a tool. 
...
main(int argc, char** argv)
{
    ...
    extern char *poptarg;
    const char* params = "a:d:D:efl:op:P:rs:t";
    int   c;
	...
	switch(c) {
	    ...
	    case 'd': d = atoi(poptarg);	// number of dimensions [2]
		      break;
	    case 'D': D = atof(poptarg);	// delay between frames [0]
		      break;
	    case 'e': cenergy = 0;		// color by energy [no]
		      break;
	    case 'f': f_flag = FALSE;		// fill star points [yes]
		      break;
	    case 'l': lmax = atof(poptarg);	// axes +/- lmax [get from data]
		      break;
	    ...
	    case 'P': rel_point_size = atof(poptarg);   // point scale factor
		      break;
	    case 'r': b_flag = FALSE;		// black background [yes]
		      break;
    ...
    // Reinterpret scaling if size reflects radius.

    if (point_mode == 2 && rel_point_size > 0)
	rel_point_size = 1.0/rel_point_size;
        
    int   gfx_counter = 0;
    while (b = get_hdyn()) {
	convert_relative_to_absolute(b);
        xhrdplot(b, scale, k, d, lmax,
