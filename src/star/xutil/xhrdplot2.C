 ...
 *    version 5:  Aug 1996   Simon Portegies Zwart email: spz.fys.ruu.nl
 ...
 local float get_point_size(float radius)
{
    ...
    if (point_scale_mode == 1)
        return max(SMALL_DOT_SIZE, base_point_size * pow(log10(radius), 2));
    else if (point_scale_mode == 2)
        return max(SMALL_DOT_SIZE, base_point_size * radius);
    else
...
local int plot_stellar_hrd(sdyn3* b, int f_flag) {
    ...
    real t_rel, m_rel, m_env, m_core, T_eff, L_eff;
    t_rel=m_rel=m_env=m_core=T_eff=L_eff=0; 	// stellar parameters
    real R_eff = 0;				//extra parameter.
       ...
       // Root will be at the start of the list if it is being displayed.
        ...
        sdyn3ptr* p = new sdyn3ptr[n_nodes+root];

        int ip = 0;
        for_all_nodes(sdyn3, b, bi) if (root || bi != b) p[ip++] = bi;
        if (ip != n_nodes+root) {
...
//		Methos for plotting HRD.
                t_rel=m_rel=m_env=m_core=T_eff=L_eff=0;
                st = bi->get_starbase()->get_star_story();
                extract_story_chapter(type, t_rel,
                          m_rel, m_env, m_core, T_eff, L_eff, *st);
...
//cerr<<"p:"<<point_size<<endl;
                ...
                float actual_point_size = get_point_size(R_eff);
...
local int plot_stars(sdyn3* b, int f_flag)
{
	...
	for_all_nodes(sdyn3, b, bi) if (root || bi->get_parent()) {
...
void xhrdplot(sdyn3* b, float scale, int k, int d, float lmax,
