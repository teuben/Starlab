//
// chydro_leapfrog.C   [ as hydro_leapfrog, for now, just for testing chydro ]
// 

//    Here the core masses are simply added, as well as the core radii,
//      and similarly for the total mass and effective radii.
//    This should be replaced by more realistic recipees.
...
void detach_node_from_general_tree(dyn & b)
{
    if(&b == NULL){
	cerr << "Warning: detach_node_from_general_tree, b is null\n";
	return;
    }
    ...
    if(parent == NULL){
	cerr << "Warning: detach_node_from_general_tree, b has no parent\n";
	return;
    }
...
cerr << "entering merge(" << bm1->get_index() << ", " << bm2->get_index()
    ...
    real  new_r_core = get_core_radius(bm1) + get_core_radius(bm2);
    real  new_m_core = get_core_mass(bm1) + get_core_mass(bm2);
    
    addchydro(bm, new_r_eff, new_r_core, new_m_core);
    
    dyn * root = bm1->get_parent(); 

    detach_node_from_general_tree(*bm1);
    detach_node_from_general_tree(*bm2);
...
main(int argc, char **argv)
{
    ...
    while ((c = pgetopt(argc, argv, "c:t:d:D:e:")) != -1)
	switch(c)
	    {
	    case 'a': a_flag = TRUE;
		      dt = atof(poptarg);
	    ...
	    case 't': t_flag = TRUE;
		      ...
		      break;
            case '?': cerr <<
		      "usage: chydro_leapfrog -t # -a # -e # -D # -d # " <<
		      "[-c \"..\"]\n" <<
