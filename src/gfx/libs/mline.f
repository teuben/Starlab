...
c     
      call routine id('mline')
      idline = 0
      go to 1
c
c-----------------------------------------------------------------------
c     
      entry dline(xarray,yarray,n,jth,jsymbl,htsym)
c     
c     As for mline, but plot dashed lines.
c     
...
c         
     ...
     &                .and.y1.ge.0..and.y1.le.slen)) then
                  if(jsymbl.ge.0.or.htsym.eq.0.)then
	              if(istar.eq.0)then
...
c     
      ...
      in0 = (x0.ge.0..and.x0.le.rlen.and.y0.ge.0..and.y0.le.slen)
c     
...
c             
     ...
     &                .and.max(y0,y1).ge.0..and.min(y0,y1).le.slen) then
c                 
