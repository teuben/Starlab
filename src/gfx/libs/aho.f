...
c	*****************************************************************
c
        external mcdxlinew		!$pragma C (mcdxlinew)
        external mcdxcolor		!$pragma C (mcdxcolor)
        external mcdxbackg		!$pragma C (mcdxbackg)
        external mcdxpoint		!$pragma C (mcdxpoint)
        external mcdxgin		!$pragma C (mcdxgin)
        external mcdxpolyf		!$pragma C (mcdxpolyf)
        external mcdxclear		!$pragma C (mcdxclear)
        external mcdxreset		!$pragma C (mcdxreset)
        external mcdxquit		!$pragma C (mcdxquit)
        external mcdxidle		!$pragma C (mcdxidle)
        external mcdxsetwin		!$pragma C (mcdxsetwin)
        external mcdxkillwin		!$pragma C (mcdxkillwin)
        external mcdxnopen		!$pragma C (mcdxnopen)
        external mcdxread_line		!$pragma C (mcdxread_line)
        external mcdxiconifywin		!$pragma C (mcdxiconifywin)
        external mcdxiconifyall		!$pragma C (mcdxiconifyall)
...
c
        if(idev.eq.1)then
            write(60,500)4,char(modulo(iwt,64)+32)
500         format(i1,a1)
...
525     if(idev.eq.1)then
            write(60,500)5,char(modulo(ipen,64)+32)
...
c
c           if pensize doesn't work, use this...
c
            i1 = (rmin+ro)*xfac+nx0
            i2 = (rmax+ro)*xfac+nx0
            j1 = (smin+so)*yfac+ny0
...
c
	...
	entry bounds
c								*************
c	Enforce/don't enforce xsize x ysize bounds		*  BOUNDED  *
...
350         format(a1)
	    ...
	    call ps page(1)
            ...
            write(6,6000)
 6000       format('Click right-hand mouse button' 
     $                  ' in window to exit X.')
            call mcdxidle
            call mcdxreset
        end if
...
c
c       HP fortran doesn't understand '\0'...
c
	call mcdxsetwincolormap(filename(1:nf) )
c
	return
c
c-----------------------------------------------------------------------
...
c
	if (mcdxnopen().gt.0) then
            if (iquit.eq.1) then
                write(6,6001)
 6001           format('Click right-hand mouse button in current' 
     $                  ' window to exit X.')
                call mcdxidle
            end if
        end if
...
350         format(a1)
	    ...
	    call ps page(1)
            ...
            write(6,'(''Click right-hand mouse button' 
     $                  ' in window to exit X.'')')
            call mcdxidle
            call mcdxreset
        end if
...
c
	if (mcdxnopen().gt.0) then
            if (iquit.eq.1) then
                write(6,
     $          '(''Click right-hand mouse button in current' 
     $                  ' window to exit X.'')')
                call mcdxidle
            end if
        end if
