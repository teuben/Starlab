...
////
//// The first page of log output gives complete information on all
			 ...
			 bool exact, bool & tree_changed)
{
	...
	for (i = 0; i < n_next; i++) {
		...
		int adjust_tree = bi->adjust_tree_structure();
...
//    if (verbose)
    ...
    // Quietly reinitialize all kepler structures, using the same
    // code as is used in get_hdyn() (hdyn.h).  (This repeats actions
    // taken by get_hdyn() on startup.)

    for_all_nodes(hdyn, b, bb)
	if (bb->get_unperturbed_timestep() > 0
	    && bb->get_elder_sister() == NULL)
	    bb->reinitialize_kepler_from_hdyn();
...
//    if (t > 0) t_log += dt_log;
...
#if 0
	// Note -- imperfect positioning, as time is still the old time,
	// not the time at the end of the step.  To be fixed...
	...
	if (t == 0 || t >= t_reinit) 
		...
		if (next_nodes[i] && next_nodes[i]->is_valid())
		    put_single_node(cout, *next_nodes[i],
				    false,
				    true);
	    }
	...
	int ds = integrate_list(b, next_nodes, n_next, exact, tree_changed);
...
main(int argc, char **argv)
{
    ...
    //--------------------------------------------------------------------
    // for unknown reasons, merlot & halley core dump at end of run if
