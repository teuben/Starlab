...
// scatt_stats_2.C: Determine merger cross-sections for 3-body scattering.
//		    Steve McMillan, Piet Hut, Charles Bailyn (May/Sep 1994)
...
static int temp_counter_1[N_MASS][N_HE][N_RHO_ZONE_MAX];
static int total_counter_1[N_MASS][N_HE][N_RHO_ZONE_MAX];
static real sigma_1[N_MASS][N_HE];
static real sigma_err_sq_1[N_MASS][N_HE];

static int temp_counter_2[N_MASS][N_HEMASS][N_RHO_ZONE_MAX];
static int total_counter_2[N_MASS][N_HEMASS][N_RHO_ZONE_MAX];
...
local void zero_temp_counter()
{
	...
	int j, k;
	for (j = 0; j < N_HE; j++)
	    for (k = 0; k < N_RHO_ZONE_MAX; k++) temp_counter_1[i][j][k] = 0;
	for (j = 0; j < N_HEMASS; j++)
	    for (k = 0; k < N_RHO_ZONE_MAX; k++)	temp_counter_2[i][j][k] = 0;
    }
}
...
local void initialize_stats()
{
	...
	int j, k;
	for (j = 0; j < N_HE; j++) {
	    sigma_1[i][j] = sigma_err_sq_1[i][j] = 0;
	    for (k = 0; k < N_RHO_ZONE_MAX; k++) total_counter_1[i][j][k] = 0;
	}
	for (j = 0; j < N_HEMASS; j++) {
	    sigma_1[i][j] = sigma_err_sq_1[i][j] = 0;
	    for (k = 0; k < N_RHO_ZONE_MAX; k++)	total_counter_2[i][j][k] = 0;
	}
    }
}
			    ...
			    sigma_out& out)
{
	...
	temp_counter_1[i][j][rho_bin]++;

	mhemerge = mmerge * ymerge;

	j = (int) (N_HEMASS * (mhemerge - M_HEMIN) / (M_HEMAX - M_HEMIN));
	if (j < 0) j = 0;
	if (j >= N_HEMASS) j = N_HEMASS - 1;

	temp_counter_2[i][j][rho_bin]++;
	...
	cerr << "m1, m2, m3 = " << 1 - prof.m2 << " " << prof.m2
	     << " " << prof.m3 << endl;
	cerr << "m_unit = " << m_unit << endl;
	cerr << "He: " << helium(m_unit*(1 - prof.m2)) << " "
	               << helium(m_unit*prof.m2) << " "
	               << helium(m_unit*prof.m3) << endl;
	cerr << "M, Y = " << mmerge << " " << ymerge << endl;
	cerr << "n, i, j, rho_bin = "
	     << n_merge << " " << i << " " << j << " " << rho_bin <<endl;
	cerr.precision(p);
#endif
    }
}
...
local void normalize_and_store_counts(sigma_out& out, real weight)
{
	...
	for (j = 0; j < N_HE; j++) {
		...
		real factor = weight * (rho_sq_max - rho_sq_min)
		                     / out.trials_per_zone;
		sigma_1[i][j] += factor * temp_counter_1[i][j][k];
		sigma_err_sq_1[i][j] += factor * factor 
		                               * temp_counter_1[i][j][k];
		total_counter_1[i][j][k] += temp_counter_1[i][j][k];
	    }
	}
	for (j = 0; j < N_HEMASS; j++) {
	    real rho_sq_min, rho_sq_max;
		...
		real factor = weight * (rho_sq_max - rho_sq_min)
		                     / out.trials_per_zone;
		sigma_2[i][j] += factor * temp_counter_2[i][j][k];
		sigma_err_sq_2[i][j] += factor * factor
		                               * temp_counter_2[i][j][k];
		total_counter_2[i][j][k] += temp_counter_2[i][j][k];
	    }
	}
    }
} 
...
main(int argc, char **argv)
    ...
    extern char *poptarg;
    int c;
    const char *param_string = "a:A:c:C:D:d:e:l:L:n:N:p:PqQs:u:U:v:V:x:";
	...
	if (stat_print > 0 && (i+1)%stat_print == 0)
	    normalize_and_print_stats(out);
    }
