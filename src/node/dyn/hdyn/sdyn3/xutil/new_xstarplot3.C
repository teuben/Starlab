/*
 *  xstarplot.C:  plot an N-body system in an X environment.
 *
 *.............................................................................
 *
...
//  PRC(b->get_index()); PRC(b->get_name()); PRL(clean_name);
...
local void set_temp_buffer(int which)
{
	...
	default:		cerr << "xstarplot: diag error...\n";
    }
...
local char check_for_input(unsigned long win, sdyn3* b,
	       ...
	       int init_flag, int& step_mode)
{
	...
	char c = check_for_input(win, b, b_flag, f_flag);

	// Special treatment required for forward/backward step modes.

	if (c == 'b' || c == 's') {		// Enter/continue step mode
...
main(int argc, char** argv)
{
    ...
    // Store snapshots in a short circular list.

#define MAX_SNAP 128

    sdyn3ptr b[MAX_SNAP];	// Array of snapshots
    int frame_id[MAX_SNAP];	// Global ids of snapshots
    frame_id[MAX_SNAP-1] = -1;

    int snap_counter = -1;	// Index of frame in snapshot array.
    int first_snap = 0;		// The usable list runs from first_snap to
    bool wrapped = false;	// snap_counter, wrapping if necessary.
    int step_mode = 0;
    int snap_display = -1;	// Index of frame currently being displayed.

    while (true) {

	// Get index of next location to be filled, and update bookeeping.
	// Definition of "next" depends on step_mode.

	if (step_mode >= 0) {

	    // Obtain the next snapshot to display.

	    if (snap_display == snap_counter) {

		// Read in a new snapshot from stdin.

		if (++snap_counter >= MAX_SNAP) {
		    snap_counter = 0;
		    wrapped = true;
		}
		...
		if (wrapped) {
		    ...
		    // Clean up the contents of the element we are
		    // about to overwrite.

		    rmtree(b[snap_counter]);
		    first_snap = snap_counter + 1;
		...
		// cerr << "read " << snap_counter
		//      << "  id = " << frame_id[snap_counter] << endl;
		...
		if (++snap_display >= MAX_SNAP) snap_display = 0;
	    ...
	    if (--snap_display < 0) snap_display = MAX_SNAP -1;

	}

	// We have a legal snapshot.  Transform and display it.

	// PRC(snap_display); PRL(frame_id[snap_display]);
    ...
    // Go into idle mode, wait to leave.

    if (--snap_counter < 0) snap_counter = MAX_SNAP - 1;
    xstarplot(b[snap_counter], scale, k, d, lmax,
	      point_mode, rel_point_size, D, cenergy,
	      b_flag, f_flag, t_flag, -1, step_mode);
}
