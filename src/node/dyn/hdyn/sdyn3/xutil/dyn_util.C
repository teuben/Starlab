...
void convert_relative_to_absolute(sdyn3* b)
{
    if (b->get_parent()) b->inc_pos(b->get_parent()->get_pos());
    for_all_daughters(sdyn3, b, bb) convert_relative_to_absolute(bb);
}

void accumulate_potential_energy(sdyn3* bj, sdyn3*bi,
				 real& epot, real& rmin, sdyn3ptr& bmin)

// Determine the potential energy of bi with respect to bj, along
// with bi's nearest neighbor and minimum neighbor distance.

{
    if (bj->get_oldest_daughter() != (sdyn3*)NULL)
	for (sdyn3 * bb = bj->get_oldest_daughter(); bb != (sdyn3*)NULL;
...
void compute_energies(sdyn3* bj, sdyn3* bi, char& c)
{
    c = default_color;
...
int nearest_index(sdyn3* b, float r, float s, int kx, int ky)
{
    real r2_min = VERY_LARGE_NUMBER;
    int index = -1;
    for_all_leaves(sdyn3, b, bb) {
...
// Return the total number of nodes BELOW the node b.
{
	...
	int  n = 0;
	for (sdyn3* daughter = b->get_oldest_daughter();
