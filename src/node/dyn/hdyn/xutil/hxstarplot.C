...
local void plot_star(hdyn *bi, float r, float s,
		     float actual_point_size, int f_flag)
{
	...
	draw_star_point(win, r, s,
			scale*actual_point_size, temp_flag);
	if (links && reset_grey)
	    lux_set_color(win, lux_lookup_color(win, "grey"));
    }
}
...
local char check_for_input(unsigned long win, hdyn* b,
	       ...
	       int init_flag, int& step_mode, bool eod)
{
	...
	sprintf(temp_buffer, "   N = %d (t = %.3f) max=%5.3f   ",
	...
	char c = check_for_input(win, b, r_flag, f_flag, eod);
...
main(int argc, char** argv)
{
    ...
    // Store snapshots in a short circular list to allow backward steps.
...
#define MAX_SNAP 512
	...
	switch(c) {
	    ...
	    case 'u': unperturbed = 1 - unperturbed;
	    	      break;
	    case 'v': verbose = true;		// verbose mode [no]
		      break;
    ...
    int * frame_id = new int[max_snap];	// Global ids of snapshots
	...
	if (step_mode >= 0) {
	    ...
	    if (!eod && snap_display == snap_counter) {
		...
		if (bbb == NULL) {
		    ...
		    cerr << "End of input data reached;"
			 << " entering step mode."
			 << endl << flush;
		    step_mode = 1;
		    eod = true;
		...
		if (!eod || step_mode > 0) snap_display++;

		if (snap_display >= max_snap) snap_display = 0;
		if (!wrapped && snap_display > snap_counter)
		    snap_display = 0;
	...
	// We have a legal snapshot.  Transform and display it.
        ...
        xstarplot(b[snap_display], scale, k, d, lmax,
		  ...
		  r_flag, f_flag, t_flag, frame_id[snap_display],
		  step_mode, eod);
