	  ...
	  int s_flag)      // symmetric timestep ?
{
	...
	if (i < n_iter && s_flag) {
	    real alpha;
	    correct_step(b, prev_new_dt, prev_new_dt);
	    real end_point_dt = the_tfp(b, eta);
	    alpha = (end_point_dt-dt)/prev_new_dt;
	    new_dt = 0.5 * (dt + end_point_dt);
	    //Piet's amaging formula
            real new_dt_piet = dt + 0.5 * (end_point_dt - dt) * (new_dt/prev_new_dt);
            // something easier to prove
            new_dt = 2*dt/(2-alpha);
            new_dt = new_dt_piet;
     	    cerr << "iter, dt = " << i << " " << new_dt <<" " << new_dt_piet<< endl;
	    if (x_flag) {
		       ...
		       print)
{
    ...
    while (t < t_end && !end_flag) {
        ...
        step(b, t, eps, eta, dt, max_dt, dt_old,end_flag, the_tfp, n_iter,
	     x_flag, s_flag);
	dt_old = dt;
...
main(int argc, char **argv)
{
	...
	switch(c)
	    ...
	    case 'z': z_flag = TRUE;
		      ...
		      break;
            case '?': cerr <<
		     "usage: low_n_evolve -t # -A # -C # -e # -D # -d # " <<
		     " etc (?!)" <<
		      "[-c \"..\"]\n" <<
		      "for t (time span), a (time step parameter), " <<
