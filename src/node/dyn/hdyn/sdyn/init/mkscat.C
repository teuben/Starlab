...
local void initialize_root(sdyn* root, real v_inf, real rho, real peri,
			   real r_init,
    ...
    // Exactly one of rho and peri should be < 0.

    if (rho*peri > 0) err_exit("Inconsistent initial conditions.");

    if (peri > 0) {
	if (v_inf > 0)
	    rho = peri * sqrt(1 + 2*m_total/(peri*v_inf*v_inf));
	else
	    rho = peri;
    }
...
local void split_particle(sdyn* current, real ecc, real sma, int planar,
    ...
    if (ecc < 0) ecc = sqrt(randinter(0,1));	// Thermal distribution
...
sdyn* mkscat(int argc, char **argv) {
    ...
    int i = 0;
    while (++i < argc)
	if (argv[i][0] == '-' && argv[i][1] == 's') seed = atoi(argv[++i]);
	...
	switch (argv[i][1]) {
	    ...
	    case 't': if (current == root)
			  initialize_root(root, v_inf, rho, peri,
					  initial_separation,
    ...
    if (current == root)
	initialize_root(root, v_inf, rho, peri, initial_separation,
			projectile_mass, projectile_radius);
    else
	split_particle(current, ecc, sma, planar,
		       mass_ratio, r1, r2);
